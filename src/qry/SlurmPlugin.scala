...
object Slurm {
  ...
  def run(bashCmd:String, execDir:Option[String]):Option[Int] = {
    ...
    // Create qsub command
    def job(passVariables:Boolean) = {
      val out = File.createTempFile("slurm", ".stdout")
      val err = File.createTempFile("slurm", ".stderr")
      out.delete
      err.delete
      List[String]( 
        // the program (sbatch)
    ...
    Thread.sleep(250)  // just so we submit at human speeds
    while (!slurmScript.exists) {
      Thread.sleep(1000)
    }
    // Run and return
    if (controller != "localhost") {
      ...
      Some(job(true).!)
//      Some(List("sbatch", "--output", "/tmp/out", "--error", "/tmp/err", "/home/gabor/test.sbatch").!)
    }
  }
...
if [[ ! -d "$LOG_DIR" ]]; then
  echo "Could not create log directory; exiting."
  exit 1
fi
